...
class CatcherAgent:
    def __init__(self, initial_position, learning_rate=0.1, discount_factor=0.9, exploration_prob=0.1):
        ...
        self.q_table = np.load('Catch_init.npy', allow_pickle=True).item()  # Q-table to store Q-values for state-action pairs
...
class RunnerAgent:
    def __init__(self, initial_position, learning_rate=0.1, discount_factor=0.9, exploration_prob=0.1):
        ...
        self.discount_factor = discount_factor
        self.exploration_prob = exploration_prob
        self.q_table = np.load('Run_init.npy', allow_pickle=True).item()  # Q-table to store Q-values for state-action pairs
...
class TagGameEnv(gym.Env):
            ...
            for runner in self.runners:
                ...
                if distance < catcher.special_power_range:  # Catcher is within range to freeze runner
                    if runner.is_frozen:
                        ...
                        catcher_rewards[self.catchers.index(catcher)] -= 1  # Reward for evading catcher                        
    ...
    def save_q_tables(self, episode):
        ...
        np.save(f'Catch_train_episode_{episode}.npy', self.catchers[0].q_table)
        np.save(f'Run_train_episode_{episode}.npy', self.runners[0].q_table)
