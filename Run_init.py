...
class TagGameEnv(gym.Env):
    ...
    def _calculate_rewards(self):
        ...
        runner_rewards = [-0.1] * len(self.runners)
...
#                        catcher_rewards[self.catcher.index(catcher)] -= 1  # Reward for evading catcher                        
    ...
    def _get_observation(self):
        ...
        distances = [self._calculate_distances(runner) for runner in self.runners]
        distance = []
        for i, runner in enumerate(self.runners):
            ...
            distance.append((x, y, z))
        return distance
        
    def _calculate_distances(self, runner):
        # Calculate distances between the catcher and the two runners.
        distances = [np.linalg.norm(runner.position - catcher.position) for catcher in self.catchers]
        return distances

    def save_q_table(self, episode):
        # Save the Q-table to a file after each episode.
        filename = f'Run_init.npy'
        np.save(filename, self.runners[0].q_table)
...
if __name__ == "__main__":
    ...
    for episode in range(1001):
        ...
        while not done:
            actions = [np.random.randint(0, 4) for _ in range(env.num_catchers)]  # Generate random actions for runners
            actions += [env.runners[i].choose_action(state[i]) for i in range(env.num_runners)]
